...
public function set_charset($charset)
{
    ...
    // Если это НЕ sqlite, выполняем SET NAMES
    if (strtolower($this->driver_name()) !== 'sqlite')
    {
...
public function list_columns($table, $like = null)
    {
        ...
        if (strtolower($this->driver_name()) === 'sqlite') {
            $q = $this->_connection->prepare("PRAGMA table_info(".$table.")");
            $q->execute();
            $raw_result = $q->fetchAll();
...
// MySQL attributes
            ...
            $column['extra']      = isset($row['Extra']) ? $row['Extra'] : null;
            $column['key']        = isset($row['Key']) ? $row['Key'] : null;
            $column['privileges'] = isset($row['Privileges']) ? $row['Privileges'] : null;
            $column['options']    = isset($column['options']) ? $column['options'] : array();
